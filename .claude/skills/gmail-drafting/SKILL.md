...
**4. Check it before it lands.** Call `lint_body` with the same `body` or `bodyBlocks` you plan to draft. It checks the visible words. Fix every error. Weigh each warning rather than ignoring it. Common ones:
...
**5. Create the draft.** `draft_reply` for a thread, `draft_new` for a fresh message, `draft_forward` to pass something on. Pass the typed `body` or `bodyBlocks` and the recipients. Everything else is computed.
